...
### Category 6: Data Resilience

#### R1. Lenient Email Validation for Extracted Data

**Priority:** Critical
**Effort:** S
**Impact:** Spam/automated emails with technically-invalid addresses (underscores in domains, leading hyphens, consecutive dots) are silently dropped during extraction, preventing classification and rule export

**Current State:**
`EmailStr` (Pydantic) enforces strict RFC 5321 validation on extracted email data. Real-world spam senders like `noreply@39._ecoenergi.online` and `CloudNotify@---SyncServi...-MtO0.autoworkscoll.com` are rejected, logged as warnings, and lost from the corpus.

**Recommendation:**
Replace `EmailStr` with lenient `str` validation on extracted data fields (Email model, Sender model). User-supplied email fields (`AppConfig.user_email`, `CorpusMetadata.user_email`) retain strict `EmailStr` validation.

---

*Recommendations generated by Claude on 2025-01-14*
*Updated: 2026-02-17 — Added R1 (Data Resilience)*
